package com.github.leoarj.algaworks.course.ej.functional.streams.api.comercial;
...
//        return maiorCodigo + 1;
        ...
        /*
        * Com streams, podemos substituir a programação imperativa acima
        * com o uso de fluxos com composição de funções e alcançar o mesmo resultado.
        *
        * - Obtém um stream (fluxo) da fonte de dados (List de Venda);
        * - Mapeia para um stream de inteiros, a partir de uma method reference de Venda;
        * - Utiliza do próprio InStream para obter o máximo valor no fluxo;
        * - Operação terminal retorna o valor ou um valor padrão caso não haja no fluxo.
        *
        * Obs.: Opcionalmente também, podemos lançar uma exception.
        */

        return vendas.stream()
                .mapToInt(Venda::getCodigo)
                .max()
                //.orElseThrow(() -> new NoSuchElementException("Não foi encontrado um código"));
                .orElse(0) + 1;
    }
